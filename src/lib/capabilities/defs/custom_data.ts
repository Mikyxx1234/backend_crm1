/**
 * Capacidade: custom_data — válvula de escape (PRD §3).
 *
 * Discriminated union por `mode` (atualmente um modo só: `freeform`).
...
export const customDataFreeformSchema = z.object({
  mode: z.literal("freeform"),
  fieldGroupKeys: z.array(z.string().trim().min(1)).default([]).meta({
...
export const customDataConfigSchema = z.discriminatedUnion("mode", [
  customDataFreeformSchema,
]);

export const CUSTOM_DATA_MODES = ["freeform"] as const;
export type CustomDataMode = (typeof CUSTOM_DATA_MODES)[number];

export const customDataCapability = defineCapability({
  key: "custom_data",
  label: "Dados personalizados",
  description:
    "Campos personalizados extras vinculados ao produto ou catálogo (válvula de escape).",
