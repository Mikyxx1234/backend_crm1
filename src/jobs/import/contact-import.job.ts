...
export async function processContactImport(
  ...
  // 1. Lê o arquivo do storage (volume compartilhado).
  const stored = await readStoredFile(organizationId, "imports", fileName);
  if (!stored) {
    await markOperationFailed(
      operationId,
      organizationId,
      `Arquivo de importação não encontrado no storage (imports/${fileName}).`,
    );
    ctx.error("Arquivo não encontrado no storage — abortando");
    return;
  }
